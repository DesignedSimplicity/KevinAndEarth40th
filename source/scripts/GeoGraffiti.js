...
GeoGraffiti.prototype.getCountry = function (id) {
	if (Number(id)) {
		for (i = 0; i < geograffiti._countries.length; i++) {
			if (geograffiti._countries[i].id == id) return geograffiti._countries[i];
		}
	}
	else if (id != null && id.length > 0) {
		id = id.toLowerCase();
		for (i = 0; i < geograffiti._countries.length; i++) {
			if (geograffiti._countries[i].key.toLowerCase() == id) return geograffiti._countries[i];
		}
	}
...
GeoGraffiti.prototype.getContinentName = function (id) {
	...
	if (id == 10)
		...
		return "oceania";
	else if (south.includes(id))
		return "south";
	else if (north.includes(id))
		return "north";
	else if (asia.includes(id))
		return "asia";
	else if (europe.includes(id))
		return "europe";
	else
		return "africa";
}
...
GeoGraffiti.prototype.getContinentCenter = function (continent) {
	var b = geograffiti.getContinentBounds(continent);
...
GeoGraffiti.prototype.setVisitedCountries = function (ids) {
	geograffiti._visited = ids;
}

GeoGraffiti.prototype.getVisitedCountries = function () {
	return geograffiti.getCountries(geograffiti._visited);
}

GeoGraffiti.prototype.isVisitedCountry = function (id) {
	return geograffiti._visited.includes(id);
}
...
GeoGraffiti.prototype.setPlaces = function (places) {
	if (places != null) geograffiti._places = places;
	return geograffiti._places;
}
...
GeoGraffiti.prototype.getPlace = function (id) {
	var count = geograffiti._places ? geograffiti._places.length : 0;
	for (i = 0; i < count; i++) {
		if (geograffiti._places[i].id == id) return geograffiti._places[i];
	}
	return geograffiti.getNullPlace();
}
